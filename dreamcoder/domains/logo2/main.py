...
from dreamcoder.domains.logo2.makeLogoTasks import makeTasks, montageTasks, drawLogo, manualLogoTasks, makeLogoUnlimitedTasks, generateLogoDataset, loadLogoDataset, sampleSupervised
from dreamcoder.domains.logo2.logoPrimitives import primitives, turtle, tangle, tlength
...
def array_to_image(arr, img_size=(28,28)):
    # Convert the one-dimensional array to a 2D array
    arr_2d = np.reshape(arr, img_size)
    # Convert the 2D array to a PIL Image
    return Image.fromarray(np.uint8(arr_2d * 255)).convert('RGB')

def create_image_table(train_tasks, test_tasks, images_per_row=5):
    # Image size and spacing
    img_size = (28, 28)
    h_spacing = 10
    v_spacing = 20
    # Determine canvas size
    canvas_width = images_per_row * img_size[0] + (images_per_row + 1) * h_spacing
    canvas_height = 2 * img_size[1] + 3 * v_spacing  # For two rows of images (train and test)

    # Create a blank canvas
    canvas = Image.new('RGB', (canvas_width, canvas_height), 'white')
    draw = ImageDraw.Draw(canvas)

    # Helper function to paste images
    def paste_images(tasks, row, label):
        for i, task in enumerate(tasks[:images_per_row]):
            img = array_to_image(task.examples[0][1])
            x = h_spacing + i * (img_size[0] + h_spacing)
            y = v_spacing + row * (img_size[1] + v_spacing)
            canvas.paste(img, (x, y))
        # Adding text label for the row
        draw.text((h_spacing, y + img_size[1] + 5), label, fill=(0, 0, 0))

    # Paste train and test images
    paste_images(train_tasks, 0, "Train Images")
    paste_images(test_tasks, 1, "Test Images")
...
def main(args):
    ...
    if task_dataset:
        ...
        canvas = create_image_table(train, test, len(train))
        canvas.show()
